...
class main_controller
{
	 ...
	 * @access public
	 */
	public function __construct(config $config, template $template, auth $auth, user $user, driver_interface $db, language $language, db_text $config_text, functions $functions, array $tables)
	{
		$this->config      = $config;
	...
	/**
	 * Controller for topicagewarning
	 *
	 * @param string     $name
	...
	{
		$blocks = [
			['name' => 'TAW_YEAR', 'amount' => $this->year],
			['name' => 'TAW_MONTH', 'amount' => $this->month],
			['name' => 'TAW_WEEK', 'amount' => $this->week],
			['name' => 'TAW_DAY', 'amount' => $this->day],
		];
